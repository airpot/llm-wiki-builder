...
description: Build and maintain durable Markdown LLM wikis with source ingest, canonical compiled pages, deterministic retrieval, context packs, semantic HTML, executable read-only MCP publication, companion Skills, validation, safe fixes, optimization reports, and retrieval benchmarks.
---
...
Use this skill to build and maintain durable, file-first Markdown LLM wikis. It supports purpose-led initialization with an explicit Delivery Contract, extraction profiles, source ingest, canonical compiled Markdown pages, deterministic retrieval, query-localized context packs, semantic HTML, executable read-only MCP bundles, wiki-specific thin companion Skills, validation, agent-mediated safe fixes, optimization reports, and retrieval benchmarks.
...
2. For new normal initialization, interview for the wiki purpose and final delivery form first. Confirm the Wiki Core Direction, at least one extraction profile, and the Delivery Contract before running `scripts/init_wiki.py` without `--draft`. Recommend the overridable default: canonical Markdown plus an executable read-only stdio MCP snapshot and a wiki-specific thin Skill.
...
5. Prefer scripts for deterministic operations:
   ...
   - Publish executable read-only MCP bundles and companion Skills with `scripts/publish_mcp_bundle.py`.
   - Benchmark retrieval with `scripts/evaluate_retrieval.py`.
6. When compiling or editing pages, keep raw source notes under `raw/` source-preserving and write profile-aware compiled pages under `wiki/`. Treat `wiki/**/*.md` as canonical compiled memory; treat context packs, semantic HTML, generated schemas, and MCP bundles under `reports/` as rebuildable generated artifacts.
...
- `references/retrieval-workflow.md`: deterministic ranking, Retrieval Contract, localized context chunks, bilingual alias/glossary expansion, profile filtering, seed-vs-context expansion, citations, and query-log schema.
- `references/lint-and-optimization.md`: validation, glossary coverage, profile coverage, MCP readiness, agent-mediated safe-fix/write-mode boundaries, reports, negative evals, and retrieval evals.
...
- `scripts/init_wiki.py`: create missing wiki structure, Wiki Core Direction, Delivery Contract, and profiles; upgrade only known generated drafts and fail on conflicts; use `--draft` only for explicit underspecified initialization.
...
- `scripts/build_context_pack.py`: generate Markdown and JSON context packs with an aggregate per-page excerpt budget and bounded section/chunk localization from deterministic retrieval hits.
- `scripts/publish_semantic_html.py`: generate collision-free clean semantic HTML pages, safe rewritten local links, stable heading/section anchors, embedded page metadata, and an index from canonical Markdown wiki pages.
- `scripts/publish_mcp_bundle.py`: generate path-confined executable read-only stdio MCP bundles, application-owned dependency metadata, portable snapshots, quality reports, schemas, and wiki-specific thin Skills under `reports/publish/mcp/`.
...
- Keep the generated MCP SDK dependency application-owned. Do not import it from builder scripts or install it automatically.
- Do not replace canonical Markdown with generated HTML, context packs, schemas, or MCP bundles; generated outputs are for consumption and publication only.
...
- Do not publish canonical pages or copied trees that resolve outside their declared root through symbolic links.
- Do not include operational logs, query logs, prior context packs, stale HTML, or source-host absolute roots in MCP snapshots.
- Do not copy canonical wiki content into a generated companion Skill; it may depend only on public MCP tools/resources.
